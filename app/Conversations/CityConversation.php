...
class CityConversation extends Conversation
{
    ...
    public function askForCity()
    {
        ...
        if(empty($client)) {
            ...
            $question = Question::create('Пожалуйста введите ваш город:');

            $this->ask($question, function (Answer $answer) {

                $client = new Client();
                $client->town = $answer->getText();
            ...
            if(empty($client->town)) {
                ...
                $question = Question::create('Ваш город (Пример: г.Шымкент): ' . $clients->town ."\n".'Новое значение:');
                $this->ask($question, function (Answer $answer) {
